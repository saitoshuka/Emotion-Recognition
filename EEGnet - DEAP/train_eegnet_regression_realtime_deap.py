...
def build_dataset_from_cache(
    ...
    if not files:
        raise RuntimeError(f"No s*.dat found in {data_dir}")
    ...
    for path in files:
        ...
        g_all.append(groups + g_offset)
        g_offset += int(groups.max()) + 1 if len(groups) else 0

    if not X_all:
        raise RuntimeError("No valid samples after loading from cache.")

    X = np.concatenate(X_all, axis=0).astype(np.float32)
    y = np.concatenate(y_all, axis=0).astype(np.float32)
    groups = np.concatenate(g_all, axis=0).astype(np.int64)
    return X, y, groups, files


def main():
    parser = argparse.ArgumentParser(description="Train EEGNet regression model for real-time circumplex (DEAP, cache-enabled)")
    ...
    parser.add_argument("--out-dir", type=str, default="realtime_eegnet_regression_deap")
    ...
    parser.add_argument("--channel-indices", type=str, default="", help="e.g. 0,1,2 ; empty means all channels in cached X")
    ...
    X, y, groups, files = build_dataset_from_cache(
